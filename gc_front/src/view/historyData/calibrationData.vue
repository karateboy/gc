...
import { getCalibrationData, getGcList, getMonitors, getMonitorTypes, } from '@/api/data';
...
export default {
    ...
    getGcList()
      .then(resp => {
        ...
        if (this.gcList.length > 0) {
          this.formItem.gc = this.gcList[0].key;
        }
      })
      .catch(err => alert(err));
    ...
    getMonitorTypes()
      .then(resp => {
        ...
        if (this.monitorTypeList.length > 0) {
          this.formItem.monitorTypes.push(this.monitorTypeList[0]._id);
        }
      })
      .catch(err => {
        alert(err);
  ...
  methods: {
    handleSubmit() {
      this.$refs.calibrationData.validate(valid => {
        if (valid) {
          this.query();
        }
      });
