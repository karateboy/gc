...
class Query @Inject()(recordOp: RecordOp,
  ...
  def alarmData(start: Long, end: Long): Action[AnyContent] = Security.Authenticated.async {
    implicit request =>
  ...
  def excelForm(pdfId: String): Action[AnyContent] = Security.Authenticated.async {
  ...
  def excelFormAr(pdfId: String): Action[AnyContent] = Security.Authenticated.async {
    val f = recordOp.getRecordWithPdfID(new ObjectId(pdfId))
    for (map <- f) yield {
      val excelFile = excelUtility.excelFormAr(map)
  ...
  def excelFormN2(pdfId: String): Action[AnyContent] = Security.Authenticated.async {
    val f = recordOp.getRecordWithPdfID(new ObjectId(pdfId))
    for (map <- f) yield {
